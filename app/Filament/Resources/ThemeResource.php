            ...
            ->actions([
                ...
                Tables\Actions\DeleteAction::make()
                    ...
                    ->action(function ($record) {
                        ...
                        if (File::exists($migrationsPath)) {
                            try {
                                \Artisan::call('migrate:rollback', [
                                    '--path' => 'themes/' . $slug . '/migrations',
                                    '--force' => true
                                ]);
                                \Log::info("Rolled back migrations for theme: {$slug}");
                            } catch (\Exception $e) {
                                \Log::warning("Failed to rollback migrations for theme {$slug}: " . $e->getMessage());
                            }
                        }
                        
                        // Delete theme files first
                        $filesDeleted = $loader->deleteTheme($slug);
                        
                        // Then delete database record
                        $record->delete();
                        
                        \Log::info("Theme {$slug} deleted successfully", [
                        ...
                        Notification::make()
                            ->title('Theme Deleted')
                            ->body("Theme {$slug} has been permanently deleted.")
                            ->success()
                            ->send();
                    })
