                ...
                Tables\Actions\DeleteAction::make()
                    ...
                    ->action(function ($record) {
                        ...
                        if (File::exists($migrationsPath)) {
                            try {
                                \Artisan::call('migrate:rollback', [
                                    '--path' => 'Modules/' . $slug . '/migrations',
                                    '--force' => true
                                ]);
                                \Log::info("Rolled back migrations for module: {$slug}");
                            } catch (\Exception $e) {
                                \Log::warning("Failed to rollback migrations for module {$slug}: " . $e->getMessage());
                            }
                        }
                        
                        // Delete module files first
                        $filesDeleted = $loader->deleteModule($slug);
                        
                        // Then delete database record
                        $record->delete();
                        
                        \Log::info("Module {$slug} deleted successfully", [
                        ...
                        Notification::make()
                            ...
                            ->body("Module {$slug} has been permanently deleted.")
                            ->success()
                            ->send();
                    })
