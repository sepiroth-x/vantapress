...
VantaPress now includes an **opt-out anonymous telemetry system** to help developers understand real-world usage patterns and prioritize features.

#### ✨ What's New in v1.0.7

**🔍 Telemetry System (NEW):**
- **VPTelemetry Module** - Anonymous usage data collection
  - Installation ID tracking (UUID-based, no personal data)
  - Version and PHP version monitoring
  - Module and theme usage statistics
  - Daily heartbeat system (max 1 ping/day)
  - Event-based pings (install, update, module changes)
- **Privacy-First Design**:
  - ❌ NO emails, usernames, passwords, or content collected
  - ✅ Only system information (version, PHP, OS, modules, themes)
  - ✅ User-controllable via Admin Panel (Settings → Telemetry)
  - ✅ Full transparency with local audit logs
  - ✅ HTTPS-only transmission
  - ✅ Rate-limited (max 10 requests/hour)
- **VPTelemetryServer Module** - Data receiver for developers
  - API endpoint for telemetry collection
  - Filament dashboard with beautiful charts
  - Installation tracking and analytics
  - Module/theme popularity statistics
  - PHP version distribution insights
  - Active/inactive installation monitoring
- **Comprehensive Documentation** - See `TELEMETRY.md` for full details

**🖥️ Villain Terminal Enhancements:**
- **Unix-Like Commands** - Terminal now supports filesystem navigation:
  - `pwd` - Print working directory
  - `cd [path]` - Change directory (supports `.`, `..`, relative, absolute paths)
  - `ls [-l]` - List directory contents (with optional detailed view)
...
- **Virtual Filesystem** - Pre-populated directory structure:
  - `/app/`, `/config/`, `/storage/`, `/public/`, `/resources/`, `/routes/`
  - Session-based current directory tracking
  - Color-coded output (cyan directories, white files, red errors)
- **Dynamic Prompt** - Shows current directory: `user@vantapress:/path$`
- **Theme Export Fix** - Resolved 404 error on theme layout downloads
  - Changed storage path from `storage/app/exports` (private) to `storage/app/public/exports` (web-accessible)
  - Download URLs now work correctly: `/storage/exports/{filename}`

**🔧 Technical Improvements:**
- Added `TelemetryService` with comprehensive data collection methods
- Created `TelemetryLog` model for local audit logging
- Implemented Filament settings page with toggle control
- Built telemetry dashboard with 4 chart widgets:
  - Stats Overview (total, active, new, pings)
  - Module usage bar chart
  - Theme usage doughnut chart
  - PHP version distribution pie chart
  - Installation timeline line chart
- Created `FileSystemCommand` for Unix terminal operations
- Updated `CommandRegistry` to support Unix commands alongside `vanta-*` commands
- Enhanced `TerminalExecutor` with current working directory (cwd) tracking

**📚 Documentation:**
- New `TELEMETRY.md` - Complete telemetry guide for users and developers
- API documentation for telemetry endpoint
- Privacy policy and opt-out instructions
- Developer guide for VPTelemetryServer module

---

## 📌 Previous Version: v1.0.6 - Installation Wizard Improvements

### 🎨 Installation Experience Enhanced

This release focuses on improving the web-based installation wizard with better UX and automatic role assignment.

#### ✨ What's New in v1.0.6

**Installation Wizard Improvements:**
- **Fixed Parse Error** - Resolved syntax error in emergency migration fix (missing semicolon)
- **Eliminated Nested Scrollbars** - Removed awkward double-scrollbar situation
- **Increased Log Visibility** - Enhanced log container heights:
  - Desktop: 400px (previously 300px)
  - Mobile: 250px (previously 150px)
- **Better Text Wrapping** - Improved overflow handling for long database names and paths
- **Responsive Design** - Better mobile experience with optimized padding and font sizes

**Admin Setup Automation:**
- **Auto Super-Admin Role** - Created admin users now automatically receive `super-admin` role
- **Smart Role Detection** - Gracefully handles cases where role tables don't exist yet
- **Better Feedback** - Clear status messages for role assignment success/failure

**Technical Fixes:**
- Fixed `database/migration-fixes/000_emergency_drop_all_menu_tables.php` line 101
- Removed nested scroll containers causing visual issues
- Improved CSS overflow handling across all installation steps
- Enhanced mobile responsiveness (480px and 768px breakpoints)

---

## 📌 Previous Version: v1.1.5-complete - IMPROVED Emergency Fix (Production Tested)
